...
BinaryOption::BinaryOption(){} // Definire meglio questo costruttore

BinaryOption::BinaryOption(Asset asset, float strike_price, float time_to_maturity, float payoff){
    this->asset = asset;
...
BinaryOption::BinaryOption(BinaryOption &option){
...
float BinaryOption::call_payoff_montecarlo_gpu(const int n_simulations){
...
float BinaryOption::put_payoff_montecarlo_gpu(const int n_simulations){
    const int N_THREADS = 256;
    const int N_PATHS = n_simulations;
...
float BinaryOption::actual_call_payoff_blackSholes() {
...
float BinaryOption::actual_put_payoff_blackSholes() {
